...
class ClientContract(unittest.TestCase):
    ...
    def test_bash_score_zero_and_parameters(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env={**os.environ,'PATH':str(bin)+os.pathsep+os.environ['PATH'],'TM_CONFIG_FILE':str(cfg),'TM_ROUTE_SCRIPT':'/dev/null','TM_ENDPOINT':'','TM_ENDPOINTS':'','CAPTURE':str(capture)}
    ...
    def test_multi_node_remember_tagging(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            res1=subprocess.run(['bash',str(ROOT/'skills/transcendence-memory/scripts/tm-remember.sh'),'测试节点记忆','--node','worker-node','--tags','infra'],capture_output=True,text=True,env=env)
            self.assertEqual(res1.returncode,0,res1.stderr)
            body1=json.loads(capture.read_text())
            tags1=body1['objects'][0]['tags']
            self.assertIn('node:worker-node',tags1)
            self.assertIn('infra',tags1)
            self.assertIn('node=worker-node',res1.stdout)
            # 2. With --no-node
