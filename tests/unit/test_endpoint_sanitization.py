...
def test_same_machine_allows_localhost() -> None:
    assert ensure_exportable_endpoint("http://127.0.0.1:8000", Topology.SAME_MACHINE) == "http://127.0.0.1:8000"


def test_local_only_hosts_are_detected() -> None:
    assert endpoint_is_local_only("http://localhost:8000")
    assert endpoint_is_local_only("http://backend:8000")
