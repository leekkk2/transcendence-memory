...
| `search --match <pattern> <query>` | Search across all containers whose name fuzzy-matches `<pattern>` | `/tm search --match team docker compose` |
...
| `containers [pattern]` | List containers, optionally filtered by a fuzzy pattern | `/tm containers team` |
...
示例：
- `/tm containers` — 列出全部
- `/tm containers team` — 列出名字里包含 `team` 的容器（大小写不敏感）
