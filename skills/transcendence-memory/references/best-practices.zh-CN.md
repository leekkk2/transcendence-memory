...
很多项目默认 container 是 `team`、`host-z` 这种通用名,日积月累塞进数千条对话备份/笔记。**新写入的精炼经验会被淹没在噪声里**:

- 实测:在 5000+ chunks 的 team 容器里写入 4 条 RN OTA 经验,用 "react native ota" 检索,top 5 全部是历史对话,新经验进不了 topk=10
...
- 默认容器(`team`/`host-z` 等)只放当前项目的临时记忆,不放长期可复用知识
