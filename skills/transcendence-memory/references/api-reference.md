...
| `container` | string | 否 | 单容器搜索，向后兼容字段（默认 `host-z`） |
...
curl -sS -X POST "${ENDPOINT}/search" \
  -H "X-API-KEY: ${API_KEY}" -H "Content-Type: application/json" \
  -d '{"query":"docker compose","container_pattern":"team","topk":5}'

# 显式列出多个容器
curl -sS -X POST "${ENDPOINT}/search" \
  -H "X-API-KEY: ${API_KEY}" -H "Content-Type: application/json" \
  -d '{"query":"deploy","containers":["team","yzjx_codex"],"topk":10}'
```
...
```json
{
  "status": "ok",
  "container": "team",
  "containers": ["team", "yzjx_claude", "yzjx_codex"],
  ...
  "results": [
    {"container": "team", "score": 0.12, "text": "..."},
...
响应示例：
```json
{"status": "updated", "id": "mem-001", "container": "host-z"}
```
...
响应示例：
```json
{"status": "deleted", "id": "mem-001", "container": "host-z"}
```
...
响应示例：
```json
{"status": "ok", "container": "host-z", "answer": "Text ingested into container host-z knowledge graph.", "mode": "insert"}
```
...
响应示例：
```json
{"status": "accepted", "container": "host-z", "filename": "document.pdf", "pid": 12345}
```
...
# 模糊匹配（大小写不敏感子串）
curl -sS "${ENDPOINT}/containers?pattern=team" -H "X-API-KEY: ${API_KEY}"

# 前缀匹配
curl -sS "${ENDPOINT}/containers?pattern=team&mode=prefix" -H "X-API-KEY: ${API_KEY}"
...
{
  "containers": [
    {"name": "team", "objects": 3237, "indexed": true, "last_modified": "2026-04-09T10:00:00Z"},
...
响应示例：
```json
{"status": "deleted", "container": "host-z"}
```
