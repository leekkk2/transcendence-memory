...
- OpenClaw memory CLI: https://docs.appdir.ai/cli/memory (HIGH confidence for status/search/index command expectations in the host ecosystem).
- OpenClaw auth and onboarding: https://docs.appdir.ai/gateway/authentication, https://docs.appdir.ai/concepts/oauth, https://docs.appdir.ai/start/wizard, https://docs.appdir.ai/start/onboarding (HIGH confidence for `apiKey + OAuth`, login/status-style UX, and local-vs-remote setup expectations).
- OpenClaw secrets and safe output: https://docs.appdir.ai/gateway/secrets, https://docs.appdir.ai/cli/dashboard, https://docs.appdir.ai/cli/configure (HIGH confidence for SecretRef behavior, redacted output, and safe configuration patterns).
- OpenClaw skill packaging: https://docs.appdir.ai/tools/creating-skills (HIGH confidence for `SKILL.md`-first skill structure expectations).
