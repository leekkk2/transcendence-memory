...
They are **operator-facing deployment assets / packaging wrapper**, not local's live unit record.
Current local live reality has been confirmed separately:
- live unit on local: `rag-everything.service`
- live `ExecStart`: `/home/user/.appdir/workspace/scripts/run_task_rag_server.sh`
- live listen: `0.0.0.0:8711`

Treat `transcendence-memory-backend` as the public wrapper name for the verified private runtime, and do not describe it as local's current live unit name unless production reality changes and is re-verified.
...
- Runtime listens on `0.0.0.0:8711` and is typically reverse-proxied as `memory.example.com -> 127.0.0.1:8711`
...
- The remaining production gap is whether local's **actual** long-running process manager state matches this wrapper exactly; until verified, document live differences in root-level long-running state files rather than inventing certainty here.
