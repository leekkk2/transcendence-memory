...
- 私有部署链路口径为：`memory.example.com` → Nginx → `127.0.0.1:8711` → `./scripts/run_task_rag_server.sh`
...
命名边界必须明确：
- **local 现网 live unit**：`rag-everything.service`
- **公开分发 wrapper 名**：`transcendence-memory-backend`

两者都指向同一条 canonical runtime 链路，但不是同一个 naming surface；不要把公开 wrapper 名写成 local 当前线上 unit 名。
